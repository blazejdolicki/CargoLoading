...
/**
 * This class opens a stage with options used by backtracking algorithm when we use set amount of pentominoes.
 * @author Basia, Jordan
 * @version 1.5
 * @date 23.01.2018
 */
public class PentominoSet extends VBox{
    /**
     * Constructor generates all options.
     * @param options an instance of BacktrackingOptionsPane
     */
    public PentominoSet(BacktrackingOptionsPane options){

        Label amount = new Label("Enter amount of parcels:");
        amount.setFont(new Font("Arial", 19));
        getChildren().add(amount);

        Label typeA = new Label("Type L");
        ...
        submit.setOnAction(new EventHandler<ActionEvent>(){
            ...
            public void handle(ActionEvent e){
                if(value.isSelected()){
                    options.solveSetAmountPentominoes(Integer.parseInt(typea.getText()), Integer.parseInt(typeb.getText()), Integer.parseInt(typec.getText()), ORDER.VALUE, Integer.parseInt(Timer.getText()));
                }
                else if(ratio.isSelected()){
                    options.solveSetAmountPentominoes(Integer.parseInt(typea.getText()), Integer.parseInt(typeb.getText()), Integer.parseInt(typec.getText()), ORDER.RATIO, Integer.parseInt(Timer.getText()));
                }
            }});
